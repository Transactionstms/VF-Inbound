<%-- 
    Document   : FacturacionVistaPrevia
    Created on : 8/03/2022, 05:14:10 PM
    Author     : luis_
--%>
...
<html>
    ...
    <body>
        <%
            try {
                ...
                try {
                    ...
                    if(tipo.equals("1")){
                       path = "D:\\Servicios\\Facturacion\\Cfdi\\VisualizacionFactura.pdf";
                    }else{
                       path = "D:\\Servicios\\Facturacion\\Cfdi\\cvd"+cve+"\\" + numFactura +".pdf";
                    }
                ...
                try {
                    ...
                    if (db.doDB(fac.consultarConceptosIngresadosPDF(serie, folio, cve))) {
                        for (String[] rowF : db.getResultado()) {
                            
                            String comentarios = rowF[9];
                            String rel_clvsat_id = rowF[4];
                            String unidad_sat_id = rowF[5];
                            String concepto_id = rowF[1];
                            String concepto_desc = rowF[2];
                            String cantidad = rowF[3];
                            String precio_unitario = rowF[6];
                            String importe_descuento = rowF[7];
                            importe = rowF[8];
                            
                            //Extraer descripción de clave unidad sat
                            if (db.doDB(fac.consultarDescripcionUnidadMedidaSat(unidad_sat_id))) {
                                for (String[] rowS : db.getResultado()) {
                                     unidad_sat_desc = rowS[0];  
                                }
                            }
                            
                            //Extraer tipo de concepto (producto/servicio)
                            if (db.doDB(fac.consultarTipoConcepto(cve,rel_clvsat_id))) {
                                for (String[] rowL : db.getResultado()) {
                                     tipoConcepto = rowL[0]; 
                                }
                            }
                            
                            //Extraer precio unitario (producto/servicio) 
                            if (db.doDB(fac.consultarPrecioUnitarioProducto(cve,rel_clvsat_id))) {
                                for (String[] rowL : db.getResultado()) {
                                     costoUnitario = Float.parseFloat(rowL[0]); 
                                }
                            }
                            
                            //Se obtiene el subtotal gral por la cantidad de conceptos ingresados
                            costoGral = Float.parseFloat(cantidad)*costoUnitario;
                            
                            //Identificar tipo de concepto para aplicar el iva correspondiente depiendo de su tasa_id
                            if (db.doDB(fac.consultarTipoConcepto(cve,rowF[1]))) {
                                for (String[] rowT : db.getResultado()) {

                                    if(rowT[0].equals("1")){  //(1=producto)
                                        subtotal = (Float.parseFloat(rowF[3])*Float.parseFloat(rowT[1]))/ivaProducto; 
                                        traslado_iva = (Float.parseFloat(rowF[3])*Float.parseFloat(rowT[1]))-subtotal;
                                    }else{                  //(2=servicio)
                                        subtotal = Float.parseFloat(rowT[1]);
                                        traslado_iva = subtotal * ivaServicio;
                                    }
                                }
                            }
                            ...
                            parametros.addCell(new Paragraph(concepto_id, espacios));      //codigo
                            parametros.addCell(new Paragraph(rel_clvsat_id, espacios));      //clave sat
                            parametros.addCell(new Paragraph(unidad_sat_desc, espacios));    //clave unidad desc
                            ...
                            parametros.addCell(new Paragraph("$ " + formatter.format(subtotal), espacios));     //importe
                            documento.add(parametros); 
                            
                            //Calcular subtotal gral
                            subtotalGral += subtotal;
                            //Calcular traslado gral
                            trasladoGral += traslado_iva;
                            //Calcular Retenciones Gral
                            retencionesGral += retenciones;
                            //Calcular Descuento Gral
                            importe_descuentoGral += descuento;
                            
                            //Calcular Total Gral
                            total = subtotalGral + trasladoGral;
                        }
                    }
                    ...
                    table60.addCell(new Paragraph("                                       Base Gravable:    $" + formatter.format(subtotalGral) + "             Impuesto:  002                                           Tasa         16.000000%           Importe:        $ " + formatter.format(trasladoGral), fontTitututu)); 
                    ...
                    table61.addCell(new Paragraph("$ " + formatter.format(subtotalGral), encabezado12));
                    ...
                    table61.addCell(new Paragraph("$ " + formatter.format(trasladoGral), encabezado12));
        ...
        %>
        <!--<div class="container"> 
            <iframe class="responsive-iframe" src="<%=request.getContextPath()%>/MostrarPDF_FACT"></iframe>
        </div>-->
