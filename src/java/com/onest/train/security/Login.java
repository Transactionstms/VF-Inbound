...
public class Login extends HttpServlet {
    ...
    private String targetOk = "/System/index.jsp";
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        String cliente = request.getParameter("hidCliente");
        
        //oc.verifyLoginN(user, password);
        Usuario root = new Usuario(user, password,cliente);
        root=BLusuario.getUsuario(user, password,cliente, 0);
        if (!auth.verifyLogin(user, password)) {
            out.println("<script> window.location.href = 'FailLogin.jsp';</script>");
        } 
            ...
            ownsession.setAttribute("login.root", root);
            ...
            if (auth.howManyConn() > 1 || auth.howManyAcct() > 1) {
                ...
                ownsession.setAttribute("db.too.many.servers", auth.howManyConn() + "");
                ownsession.setAttribute("cliente.login", cliente);
                ownsession.setAttribute("login.too.many.accounts", auth.howManyAcct() + "");
            ...
            if (factory.menuIsOk()) {
                ...
                dispacher = context.getRequestDispatcher(targetOk);
            }*/
            dispacher = context.getRequestDispatcher((auth.howManyConn() > 1 ? targetOkButTooManyServers : targetOk));
